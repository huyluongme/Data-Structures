/*
	...
	Tim thay sinh vien B1804965. Thong tin sinh vien:
	B1804965 - Nguyen Thai Anh - 3.00 - 2.50 - 2.00
*/
#include <stdio.h>
#include <string.h>
...
int main() {
	...
	Position P = tim(x, L);
	if (P != vtsauCuoi(L))
		printf("Tim thay sinh vien %s. Thong tin sinh vien:\n%s - %s - %.2f - %.2f - %.2f", x, retrieve(P, L).MSSV, retrieve(P, L).HoTen, retrieve(P, L).DiemLT, retrieve(P, L).DiemTH1, retrieve(P, L).DiemTH2);
	else
